...
function getContentBlocks(markdown, originalPath, options) {
	return contentBlocks.getBlocks(markdown)
		.reduce((collection, path) => {
			...
			let content = contentStore.getContent(path).content // should be method returning just content, not contentblocked markdown
...
class TextContent extends Content {
	...
	_getMarkdown() {
		const markdown = getMarkdown(this.content, this.originalPath, { imagePath: `/${this.assetsFolder}/` })
