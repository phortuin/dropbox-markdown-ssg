...
function getContentBlocks(markdown, originalPath, options) {
	return contentBlocks.getBlocks(markdown)
		.reduce((collection, _path) => {
			// Prevent endless recursion when a file references itself as a
			// content block
			if (`/${_path}` === originalPath) {
				return collection
			}
			// Note:
			// We don’t support fully relative paths, as iA Writer does.
			//
			// Store lookup path should be lowercase, since all files are
			// referenced by their file.path_lower property. Even if the file
			// is called `File.md` on the file system and correctly referenced
			// in a content block with `/File.md`, we still store and lookup
			// with its lowercased variant.
			let searchPath = path.join(path.dirname(originalPath), _path).toLowerCase()
...
class DropboxFile {
	...
	toProps() {
		const assetsTarget = path.sep.concat(path.join(this.assetsFolder, path.dirname(this.originalPath)))
		const markdown = getMarkdown(this.content, this.originalPath, { imagePath: assetsTarget })
		const parent = this.getParent() // nunjucks doet object.keys(obj) dus _of_ je this.bla is allemaal dik in orde, _of_ call gewoon myobj.toJSON() of zo
		const folder = this._getFolder()
